...
const downloadSourceCode = async () => {
  ...
  let accounts = localStorage.getItem("ctf-flashloan");
        ...
        <main className="relative pt-8 pb-3">
          <div className="py-3">
            ...
            <div className="mt-4">
              ...
              <br></br>
              Steal the money to capture the flag!
            </div>
...
const NextStep = () => {
  ...
  async function deployNewInstance() {
    ...
    if (anchorWallet?.publicKey) {
      ...
      const message = `Setup Flashloan Challanage. Click Sign or Approve only means you have proved this wallet is owned by you.`;
      ...
      console.log("signedMessage.toString() :: ", signedMessage);
      try {
        fetch(`${SERVER_URL}/flashloan/setup/`, {
          method: "POST",
          headers: new Headers({
            "Content-Type": "application/json",
          }),
          ...
          .then((data) => {
            ...
            if (data) {
              console.log("data :: ", data);
              localStorage.setItem("ctf-flashloan", JSON.stringify(data));
              setisInstanceDeployedState(true);
  ...
  const getFlag = async () => {
    ...
    if (wallet) {
      ...
      let accounts = JSON.parse(localStorage.getItem("ctf-flashloan")!);
      ...
      try {
        ...
        if (pool_balance.value.amount <= "2") {
          setPawned(true);
          setOpen(true);
        } else {
